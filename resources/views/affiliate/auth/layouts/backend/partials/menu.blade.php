    ...
    <div class="row">
        <div class="col-lg-12 p-0">
            <div
                ...
                @php
                    if (!Cache::has('language_list')) {
                        $LanguageList = getLanguageList(); // Run this only if the cache is missing
                    } else {
                        $LanguageList = Cache::get('language_list');
                    }

                    $logoPath = asset(Settings('logo'));
                    $logoData = Cache::rememberForever('logo_data', function () use ($logoPath) {
                        try {
                            return file_get_contents($logoPath);
                        } catch (\Exception $e) {
                            return '';
                        }
                    });

                    // dd($logoData, '$LanguageList', $logoPath, $type, $totalMessage, $user);
                @endphp
                <!-- test load 5-->

                <!-- test load 4-->

                <input type="hidden" id="logo_img" value="{{ base64_encode($logoData) }}">
                <input type="hidden" id="logo_title" value="{{ Settings('company_name') }}">
                <div class="small_logo_crm d-lg-none">
                    <a href="{{ route('affiliate.index') }}"> <img src="{{ asset(Settings('logo')) }}" alt=""></a>
                </div>
                ...
                <div class="d-flex align-items-center">
                    @if ($user->sidebar != 1)
                        <a class="large_logo   header_logo_area" href="{{ route('affiliate.index') }}">
                ...
                <div class="header_right d-flex justify-content-between align-items-center">
                            ...
                            <select name="code" id="language_code"
                                ...
                                @foreach ($LanguageList as $key => $language)
                                    <option value="{{ route('changeLanguage', $language->code) }}"
                                        ...
                                        @if ($user->language_code == $language->code) selected @endif>{{ $language->native }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <ul class="header_notification_warp d-flex align-items-center gap-25">
                        ...
                        <li class="nav-item" data-bs-toggle="tooltip" data-bs-placement="bottom"
                            title="{{ __('common.Visit Website') }}" data-bs-custom-class="tooltip-zindex">
                            <a target="_blank" class="visit_website_btn" href="{{ route('affiliate.index') }}">
                            ...
                            <button class="nav-layout-toggler   p-0">
                                @if ($user->sidebar != 1)
                                    <a href="{{ route('changeMenuStyle', 'sidebar') }}">
                                        <svg class="sidebar" width="24" height="24" viewBox="0 0 24 24"
                                ...
                                <div class="icons">
                                    @if ($user->dark_mode == 1)
                                        <a href="{{ route('changeDarkMode', 'light') }}">
                                            <svg width="24" class="sun" height="24" viewBox="0 0 24 24"
                                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                    ...
                                    @else
                                        <a href="{{ route('changeDarkMode', 'dark') }}">
                                            <svg width="24" class="moon" height="24" viewBox="0 0 24 24"
                                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                ...
                                <div class="nofity_footer">
                                    ...
                                    @if (permissionCheck('notification_setup_list'))
                                        <div class="submit_button text-center ">
                                            <a href="{{ route('notification_setup_list') }}" class="pt-3"
                                                style="    color: #415094!important;">
                            ...
                            <li class="scroll_notification_list">
                                @if (env('BROADCAST_DRIVER') == null)
                                    <jquery-notification-component
                                        ...
                                        :mark_all_as_read_url="{{ json_encode(route('chat.notification.allRead')) }}"></jquery-notification-component>
                                @else
                                    <notification-component
                                        ...
                                        :redirect_url="{{ json_encode(route('chat.index')) }}"
                                        :asset_type="{{ json_encode('/public') }}"
                                        :mark_all_as_read_url="{{ json_encode(route('chat.notification.allRead')) }}"></notification-component>
                                @endif
                            </li>
                        @endif
                    ...
                    <div class="profile_info">
                        ...
                        <div class="profile_info_iner">
                            ...
                            <div class="profile_info_details">
                                ...
                                @endif
                                @if (
                                    (isModuleActive('LmsSaas') && $user->is_saas_admin == 1) ||
                                        (isModuleActive('LmsSaasMD') && $user->is_saas_admin == 1))
                                    <a href="{{ route('saas_panel') }}" class="px-4 py-2"
                                ...
                                <div class="log-out-link-container">
                                    <a href="{{ route('logout') }}" class="px-4 py-2 log-out-link"
                                        onclick="event.preventDefault();
