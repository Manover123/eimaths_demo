    ...
    <div class="login_wrapper">
        <div class="login_wrapper_left">
            ...
            <div class="login_wrapper_content">
                <h4>ยินดีต้อนรับกลับ กรุณาเข้าสู่ระบบ<br>เข้าสู่บัญชีของคุณ </h4>
                        ...
                        <div class="col-12">
                            <div class="input-group custom_group_field">
                                ...
                                <input type="email" value="{{ old('email') }}"
                                    ...
                                    placeholder="กรอกอีเมล" name="email" aria-label="Username"
                        ...
                        <div class="col-12 mt_20">
                            <div class="input-group custom_group_field">
                                ...
                                <input type="password" name="password" class="form-control"
                                    placeholder="กรอกอีรหัสผ่าน" aria-label="password"
                        ...
                        <div class="col-12 mt_20">
                            <div class="remember_forgot_pass d-flex justify-content-between">
                                <label class="primary_checkbox d-flex">
                                    ...
                                    <span class="text-drak">จดจำฉัน</span>
                                </label>
                                @if (Settings('allow_force_logout'))
                                    <label class="primary_checkbox d-flex">
                                        <input type="checkbox" name="force" {{ old('force') ? 'checked' : '' }}
                                            value="1">
                                        <span class="checkmark mr_15"></span>
                                        <span class="label_name">{{ __('auth.Force login') }}</span>
                                    </label>
                                @endif
                                {{-- <a href="{{ route('affiliate.registration') }}" class="text-dark"> สมัคสมาชิก </a> --}}
                                {{-- <a href="#" <a href="{{route('SendPasswordResetLink')}}"
                                    class="forgot_pass ">ลืมรหัสผ่าน?</a> --}}
                            </div>
                        </div>
                        <div class="col-12">
                            ...
                            @if (saasEnv('NOCAPTCHA_FOR_LOGIN') == 'true' && saasEnv('NOCAPTCHA_IS_INVISIBLE') == 'true')
                                <button type="button" class="g-recaptcha theme_btn text-center w-100"
                                    ...
                                    data-callback="onSubmit" class="theme_btn text-center w-100">
                                    เข้าสู่ระบบ</button>
                            ...
                            @else
                                <button type="submit" class="theme_btn text-center w-100">
                                    เข้าสู่ระบบ</button>
                            @endif
                            <a href="{{ route('affiliate.registration') }}" class="btn mt-3 theme_btn text-center w-100">สมัครสมาชิก</a>
                        </div>
                    </div>
                </form>
            </div>
            @if (Settings('student_reg') == 1 && saasPlanCheck('student') == false)
                <h5 class="shitch_text mb-0">{{ __('frontend.Don’t have an account') }}? <a
                        href="{{ route('register') }}">
                        {{ __('common.Register') }}
                    </a></h5>
            @endif
