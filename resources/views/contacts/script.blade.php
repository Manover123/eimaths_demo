<script>
    $(document).ready(function() {
        ...
        $(".rbooks2").change(function() {
            let level = $('#ReceiptLevel2').val();
            let term = $('#ReceiptTerm2').val();
            let book = $('#ReceiptBook').val();
            let book2 = $('#ReceiptBook2').val();
            ...
            if (level.length !== 0 && term.length !== 0) {
                $.ajax({
                    ...
                    data: {
                        ...
                        level2: $('#ReceiptLevel2').val()[0],
                        term2: $('#ReceiptTerm2').val()[0],
                        book: $('#ReceiptBook').val()[0],
                        book2: $('#ReceiptBook2').val()[0],
                        _token: token,
                    },
                    async: false,
                    success: function(res) {
        ...
        $(".books2").change(function() {
            let level = $('#AddLevel2').val();
            let term = $('#AddTerm2').val();
            let book = $('#AddBook').val();
            let book2 = $('#AddBook2').val();
        ...
        $('#SearchButtons').on('click', function() {
            ...
            if (searchType !== '' && keyword !== '') {
                ...
                } else if (searchType === '2' || searchType === '7') {
        ...
        $('#SubmitCreateForm').click(function(e) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    code: $('#AddCode').val(),
                    free_course: $('#AddFreeCourse').val(),
                    free_course_reason: $('#AddFreeCourseReason').val(),
                    name: $('#AddName').val(),
                    ref: $('#AddReferral').val(),
        ...
        $(document).on('click', '.btn-fastHistory', function() {
            const studentId = $(this).data('id');

            $.get(`/contacts/${studentId}/fast-history`, function(res) {
                console.log(res.data);
                toastr.success('บันทึกประวัติการเรียนเรียบร้อยแล้ว');
                if ($.fn.DataTable.isDataTable('#Listview')) {
                    $('#Listview').DataTable().ajax.reload(null, false);
                }
                // TODO: render modal/table
            }).fail(function(xhr) {
                if (xhr.responseJSON && xhr.responseJSON.message) {
                    toastr.error(xhr.responseJSON.message);
                } else {
                    toastr.error('Loading fast history failed');
                }
            });
        });
        ...
        $(document).on('click', '#getEditData', function(e) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    $('#EditFreeCourse').val(res.data.free_course);
        ...
        $('#SubmitEditForm').click(function(e) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    code: $('#EditCode').val(),
                    free_course: $('#EditFreeCourse').val(),
                    name: $('#EditName').val(),
        ...
        $(document).on('change', '#s_student', function(e) {
            e.preventDefault();

            var std_id = $(this).val();
            //get referrent form std
            $.ajax({
                url: "contacts/getRef/" + std_id,
                method: 'GET',
                success: function(res) {
        ...
        $('#ReceiptCreateForm').click(function(e) {
            ...
            $.ajax({
                ...
                data: {
                    ref: $('#getRef').val(),
                    free_course: $('#AddRcFreeCourse').val(),
                    free_course_reason: $('#AddFreeCourseReason').val(),
                ...
                error: function(xhr) { // Handle exception errors here
                    // console.error(xhr.responseText); // Debugging

                    let errorMsg = "An unexpected error occurred!";
                    if (xhr.responseJSON && xhr.responseJSON.message) {
                        errorMsg = xhr.responseJSON
                            .message; // Use Laravel exception message
                    }
